...
def hypothesis_testing(healthy_samples, cancerous_samples):
    ...
    for i in range(len(healthy_samples)):
        ...
        gene_stats_summary.append({"Hugo_Symbol": cancerous_samples[i][0], "T_Paired": float(t_paired), "P_Paired": float(p_paired), "T_Independent": float(t_independent), "P_Independent": float(p_independent)})
    ...
    return [independent_degs, paired_degs, gene_stats_summary]